...
begin
  ...
  if not exists (select 1 from public.driver_messages where driver_id = v_driver and tipo = 'aviso') then
    insert into public.driver_messages(driver_id, tipo, texto) values
      (v_driver, 'aviso', 'Buenos días, Carlos. Prioriza la entrega de Café Central antes de las 10:30. Recuerda registrar la foto de respaldo en cada entrega. ¡Gracias por tu compromiso!');
  end if;
